...
public class NPCBuildingPlacement : MonoBehaviour {
		...
		if (PendingBuildings.Count > 0) {
			...
			if (PendingBuildings [0].CenterID < FactionMgr.BuildingCenters.Count) {
				if (FactionMgr.BuildingCenters [PendingBuildings [0].CenterID] != null) {
					...
					if (Vector3.Distance (PendingBuildings [0].BuildingToAdd.transform.position, FactionMgr.BuildingCenters [PendingBuildings [0].CenterID].transform.position) > FactionMgr.BuildingCenters [PendingBuildings [0].CenterID].CurrentCenter.Size) {
	...
	public void ConstructBuilding (Building NeededBuilding)
	{
		...
		if (Amount > 0) {
			//Create some.
			CreateBuilders (NeededBuilding, NeededBuilding.MaxBuilders, NeededBuilding.transform.position);
		} else {
	...
	public void CreateBuilders (Building Target, int Amount, Vector3 TargetArea)
	{		
		int i = 0;
		int j = 0;
		...
		if (Amount > 0 && BuilderCreators.Count > 0) {
			...
			if (ID >= 0) {
				...
				//COuldn't produce builders due to lack of resources:
			}
